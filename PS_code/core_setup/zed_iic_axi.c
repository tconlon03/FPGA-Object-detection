...
int zed_iic_axi_init( zed_iic_t *pIIC, char szName[], Xuint32 CoreAddress )
{
   ...
   //if ( pContext == NULL )
   //{
   //   print("Failed to allocate context data for FMC-IIC-AXI implementation\n\r" );
   //   return 0;
   //}
   ...
   {
      print("Failed to initialize I2C chain\n\r" );
      return 0;
   }
  ...
  do
  {
    ...
    //print("[%s] Xil_In8(pContext->CoreAddress + XIIC_SR_REG_OFFSET) => 0x%02X\n\r", pContext->szName, StatusReg );
...
/***** This function checks whether a monitor is connected *****
 *
 *
 *
 *
 *
 */
int check_hdmi_hpd_status(zed_iic_t *IicPs)
{
	int num_bytes;
	u8 data = 0x00;

	//
	num_bytes = zed_iic_axi_IicRead(IicPs, 0x39, ADV7511_HDP_REG_ADDR, &data, 1);
	if (num_bytes < 1) {
		return XST_FAILURE;
	}
